...
export default function DashboardPage() {
  ...
  /* Top jobs */
  const topJobs = [...jobs].sort((a, b) => b.candidateCount - a.candidateCount).slice(0, 5);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <div style={{ fontSize: 30, fontWeight: 700, color: "#111827", lineHeight: 1 }}>
                  {card.value}
                  <span style={{ fontSize: 14, fontWeight: 400, color: "#94a3b8", marginLeft: 4 }}>{card.suffix}</span>
                </div>
              ...
              <div
                style={{
                  ...
                  fontSize: 22, background: card.color + "12",
                }}
              >
                {card.icon}
              </div>
      ...
      {/* Main grid: funnel + todos + hot jobs */}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div className="section-card">
            ...
            <table className="data-table">
              <thead>
                <tr>
                  <th>#</th><th>岗位</th><th>部门</th><th>候选人</th><th>状态</th>
                </tr>
              </thead>
              <tbody>
                {topJobs.map((j, i) => (
                  <tr key={j.id} style={{ cursor: "pointer" }} onClick={() => router.push("/jobs")}>
                    ...
                    <td>
                      <span style={{ fontWeight: 700, color: "#111827" }}>{j.candidateCount}</span>
                    </td>
                    <td>
                      <span className={`badge-chip ${j.status === "open" ? "status-pass" : j.status === "closed" ? "status-reject" : "status-pending"}`}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {todoItems.map((t) => (
                ...
                >
                  ...
                  <span style={{ fontSize: 22, fontWeight: 700, color: "#3b82f6" }}>{t.count}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {stats.recentActivities.map((act, idx) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "10px 0",
                    borderBottom: idx < stats.recentActivities.length - 1 ? "1px solid #f1f5f9" : "none",
                  }}
                >
                  <div className="activity-dot" style={{ background: typeColors[act.type] || "#94a3b8", marginTop: 6 }} />
                  <div style={{ flex: 1 }}>
